...
// ----------------- CONFIG -----------------
const SRC_DIR = "src/assets/gallery"; // contains project folders
const OUT_DIR = "public/gallery"; // contains per-project folders with variants
...
function imageIdOf(file) {
  // id per image (inside a project folder)
...
async function run() {
  ...
  // project folders = directories inside SRC_DIR
  ...
  for (const projectFolderName of projectDirs) {
    const projectTitle = projectFolderName;
    const projectId = slugify(projectFolderName);

    const projectSrcDir = join(SRC_DIR, projectFolderName);
    ...
    for (const filename of files) {
      const srcPath = join(projectSrcDir, filename);
      ...
      if (!intrinsicWidth || !intrinsicHeight) {
        console.warn(`Skipping (no dimensions): ${projectFolderName}/${filename}`);
        continue;
      }
